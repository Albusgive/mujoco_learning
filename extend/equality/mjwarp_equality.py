...
_VIEWER_GLOBAL_STATE = {"running": True, "step_once": False, "eq_active": True, "reset_active": False}


def key_callback(key: int) -> None:
  if key == 32:  # Space bar
    _VIEWER_GLOBAL_STATE["eq_active"] = not _VIEWER_GLOBAL_STATE["eq_active"]
    logging.info("EQ_ACTIVE = %s", _VIEWER_GLOBAL_STATE["eq_active"])
  elif key == 259:  # Backspace
    _VIEWER_GLOBAL_STATE["reset_active"] = True
  elif key == 82:  # 'r'
...
def _main(argv: Sequence[str]) -> None:
  ...
  body_id = mujoco.mj_name2id(mjm, mujoco.mjtObj.mjOBJ_BODY, 'box000')
  joint_id = mjm.body_jntadr[body_id]
  qvel_adr = mjm.jnt_dofadr[joint_id]
  mjd.qvel[qvel_adr : qvel_adr + 6] = [80.0, 0.0, 0.0, 0.0, 10.0, 0.0]

  # Get sensor id for wall force
  sensor_id = mujoco.mj_name2id(mjm, mujoco.mjtObj.mjOBJ_SENSOR, 'wall_force')
  ...
  if _ENGINE.value == EngineOptions.C:
    ...
    wp.copy(d.eq_active, wp.array([mjd.eq_active[:len(d.eq_active)].astype(np.int32)]))
  ...
  with mujoco.viewer.launch_passive(mjm, mjd, key_callback=key_callback) as viewer:
    ...
    while True:
      ...
      if _ENGINE.value == EngineOptions.C:
        ...
        # Add equality logic
        wall_force = mjd.sensordata[sensor_id : sensor_id + 3]
        if not _VIEWER_GLOBAL_STATE["eq_active"] and np.linalg.norm(wall_force) > 0.01:
          mjd.eq_active[:] = 0
      else:  # mjwarp
        ...
        if not _VIEWER_GLOBAL_STATE["eq_active"] and np.linalg.norm(wall_force) > 0.01:
          mjd.eq_active[:] = 0
          wp.copy(d.eq_active, wp.array([mjd.eq_active.astype(np.int32)]))

      viewer.sync()

      if _VIEWER_GLOBAL_STATE["reset_active"]:
        mjd.qvel[qvel_adr : qvel_adr + 6] = [80.0, 0.0, 0.0, 0.0, 10.0, 0.0]
        if _ENGINE.value != EngineOptions.C:
          wp.copy(d.qvel, wp.array([mjd.qvel.astype(np.float32)]))
        _VIEWER_GLOBAL_STATE["reset_active"] = False
